"""
Generator szczegolu szuflady -> drawer-detail.html (runda 9.1)
...
PART_KIND = {
    "drawer-%d%d-front" % (LI, BAY): "front",
    "drawer-%d%d-side-L" % (LI, BAY): "side",
    "drawer-%d%d-side-R" % (LI, BAY): "side",
    "drawer-%d%d-back" % (LI, BAY): "back",
    "drawer-%d%d-bottom" % (LI, BAY): "bottom",
}
...
def parts_payload():
    ...
    for part in drawer_parts():
        add(PART_KIND[part.name], *part.triangles())
...
<header>
  <p class="eyebrow">CNC Furniture Studio &middot; runda 9.1 &middot; szczegol szuflady</p>
  ...
  5 elementow w sklejce 18/4 mm, plus 12 punktow wiercenia pod wkrety mocujace
  prowadnice: 6 w lico sasiadujacych pionow ("corpus"), 6 w bok korpusu szuflady
  ("drawer"). Szare plyty w podgladzie 3D to fragment lica pionow - odniesienie,
  nie czesc szuflady.</p>
</header>
...
<section>
  <div class="hdr"><h2><span class="num">03</span>Elementy szuflady</h2></div>
  %s
</section>

<section>
  <div class="hdr"><h2><span class="num">04</span>Wspolrzedne wiercenia (ta para)</h2>
...
<footer class="foot">Regal R150 &middot; runda 9.1 &middot; wygenerowano z
design/drawer_detail_gen.py (ta sama geometria co design/shelf_model.py,
zero recznego przepisywania wymiarow)</footer>
</div>
<script>%s</script>
""" % (pv.CSS, LI, BAY, layer_btns, view_drill_pattern(), table, drill_table, js)
